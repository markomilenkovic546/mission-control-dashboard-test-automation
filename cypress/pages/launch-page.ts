  ...
  constructor() {
    this.elements = {
      ...
      missionForm: {
        ...
        launchMissionButton: () => cy.get('button:contains("Launch Mission ✔")'),
      },
    };
    this.actions = {
      headerNavigation: {
      ...
      missionForm: {
        typeLaunchDateInput: (date) => this.elements.missionForm.launchDateInput().type(date),
        typeMissionNameInput: (missionName) => this.elements.missionForm.missionNameInput().type(missionName),
        typeRocketTypeInput: (rocketType) => this.elements.missionForm.rocketTypeInput().type(rocketType),
        selectDestinationExpoplanetDropdown: (option) => this.elements.missionForm.destinationExpoplanetDropdown().select(option),
        clearMissionNameInput: () => this.elements.missionForm.missionNameInput().clear(),
        clearRocketTypeInput: () => this.elements.missionForm.rocketTypeInput().clear(),
        clickOnLaunchMissionButton: () => this.elements.missionForm.launchMissionButton().click(),
      },
    };
  }
}
