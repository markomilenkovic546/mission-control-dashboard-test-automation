export default class HistoryPage {
  elements: {
    ...
    missionHistoryTable: {
      ...
      missionCustomers: (rowIndex: number) => Cypress.Chainable<JQuery<HTMLElement>>;
    };
  };
    ...
    this.elements = {
      ...
      missionHistoryTable: {
        ...
        missionStatus: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(0).find("span");
        },
        missionName: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(1);
        },
        missionNumber: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(2);
        },
        missionDate: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(3);
        },
        missionRocket: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(4);
        },
        missionCustomers: (rowIndex) => {
          return this.elements.missionHistoryTable.missionRow(rowIndex).find("td").eq(5);
        },
      },
    };
